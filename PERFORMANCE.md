...
1. **Fix PWA/offline** — Done: `offline.html` (no broken `style.css`), `sw.js` ASSETS (real CSS), and precached image paths aligned to existing files.
...
- **LCP (Largest Contentful Paint)**  
  - Preload hero image: `<link rel="preload" as="image" href="images/Me%20Suit%20with%20white%20bg1.png" type="image/png">`.  
...
- **Images**  
  - Hero uses `<picture>` with explicit dimensions and `fetchpriority`; project images use `loading="lazy"` (unchanged).
...
- **Cache**  
  - GitHub Pages sends cache headers; the service worker adds offline caching. Bump `CACHE_NAME` in `sw.js` when app shell assets change (`index.html`, `offline.html`, `css/*`, `script.js`, `manifest.json`, or precached images).

### Release verification checklist

Before or after each deploy, run:

1. **Local:** `npm ci && npm run verify` (HTML/CSS/JS lint plus asset-reference validation).
